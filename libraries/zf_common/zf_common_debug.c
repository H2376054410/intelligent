...
#endif
#if DEBUG_UART_USE_INTERRUPT                                                    // 如果启用 debug uart 接收中断
uint8                       debug_uart_output_buffer[DEBUG_RING_BUFFER_LEN];           // 数据存放数组
...
void debug_UART4_interrupr_handler (void)//位姿矫正
{
	if(zf_debug_init_flag)
  {
		if(position_uart4_flag==0)		//判断是图片检测还是黄线检测，这里是图片检测
		{
			uart_query_byte(UART_4, &debug_uart4_data);                    // 读取串口数据'
...
void debug_init (void)
{
    ...
    uart_init(
	    ...
	    uart_init(
        UART_4,                                                       // 在 zf_common_debug.h 中查看对应值
        DEBUG_UART_BAUDRATE,                                                    // 在 zf_common_debug.h 中查看对应值
        UART4_TX_C16,                                                      // 在 zf_common_debug.h 中查看对应值
        UART4_RX_C17);                                                     // 在 zf_common_debug.h 中查看对应值
